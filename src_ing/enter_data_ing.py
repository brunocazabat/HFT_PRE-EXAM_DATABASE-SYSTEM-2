...
def enter_data_ing(self):
    ...
    self.photo_button = ttk.Button(self.addData_Window, text='Browse',
                                   command=lambda: self.add_photo(self.addData_Window)).grid(row=3, column=1)
...
def add_data_ing(self):
    try:
        parameters = (
            self.id.get(), self.ingredient_name.get(), self.photo_label['text'].rstrip())
        self.curs.execute("INSERT INTO ingredients VALUES (%s, %s)", parameters)
        self.conn.commit()
